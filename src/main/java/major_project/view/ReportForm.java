...
public class ReportForm extends GridPane {
    ...
    public ReportForm(AppWindow window) {
        this.window = window;
    ...
    private void initNewWindow() {
        ...
        newWindow.initOwner(window.scene().getWindow());
        newWindow.show();
    }
    ...
    /**
     * Iterates through all the fields and raises an error if one is not completed.
     * @param event
     */
    private void submitButtonAction(ActionEvent event) {
        ...
        String content = window.viewer().getCurrentView().container().getEmailOutput();

        if (window.model().sendReport(toEmail, toName, fromEmail, fromName, subject, content)) {
